...
from app.learner_store import get_state, save_state
...
if _ui.is_dir():
    app.mount("/ui", StaticFiles(directory=str(_ui), html=True), name="tutor-ui")
...
async def explain_topic_legacy(
    body: ExplainRequest,
    request: Request,
    analogies: bool = Query(True, description="Include real-world analogies"),
    _rl: None = Depends(_rate_limit),
    ...
    return await explain_topic(body.topic, request, analogies=analogies, _rl=_rl)
...
class LearnerStateBody(BaseModel):
    user_id: str = Field(..., min_length=1)
    known: list[str] = Field(default_factory=list)
    mistakes: list[str] = Field(default_factory=list)


@app.get("/v1/learner/{user_id}", tags=["learner"])
async def learner_get(user_id: str, _rl: None = Depends(_rate_limit)) -> dict:
    return get_state(user_id)


@app.put("/v1/learner", tags=["learner"])
async def learner_put(body: LearnerStateBody, _rl: None = Depends(_rate_limit)) -> dict:
    return save_state(body.user_id, body.known, body.mistakes)


@app.get("/v1/graph/export", tags=["knowledge"])
async def graph_export(_rl: None = Depends(_rate_limit)) -> dict:
    """Export nodes/edges for client-side graph visualization."""
    graph = get_knowledge_graph()
    concepts = graph.list_concepts()
    edges = []
    for c in concepts:
        cid = c["id"]
        for p in graph.get_prerequisites(cid):
            edges.append({"from": p, "to": cid, "type": "prerequisite"})
